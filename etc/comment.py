...
def filter_change(change):
    ...
    return filename.find('source/') == 0 and status in ['added', 'renamed']
...
changes = [change for change in changes if change['status'] == 'renamed'] + [change for change in changes if change['status'] == 'added']
