...
keep-spawn-loaded                         boolean   true        If the spawn should stay loaded with no players. Has no effect in global, it doesn't matter if a config isnt enabled either. Falls always back to the world config.
leaf-decay                                boolean   true        If enabled, allows natural leaf decay.
load-on-startup                           boolean   true        If the world should be loaded on startup. Has no effect in global, it doesn't matter if a config isnt enabled either. Falls always back to the world config.
