...
All plugins wishing to make changes to a worlds generator must register a :javadoc:`org.spongepowered.api.world.gen.
WorldGeneratorModifier`. These modifiers are registered globally with a unique id, which must be added to the config of
a world by a server admin. With the id specified in the world config, a ``WorldGeneratorModifier`` will be
...
As you can see, ``WorldGeneratorModifier`` has three methods which we override. :javadoc:`org.spongepowered.api.
CatalogType#getId()` must be overridden to return a constant and unique identifier for your ``WorldGeneratorModifier``,
this is the identifier which will be used in the world configuration to specify which worlds your modifier should be
applied to. :javadoc:`org.spongepowered.api.CatalogType#getName()` must be overriden with a constant and simple
...
The :javadoc:`org.spongepowered.api.world.WorldCreationSettings` and a :javadoc:`org.spongepowered.api.data.
DataContainer` of additional properties for the world are passed to this method in order to give context for your
changes. For instance, you can use the ``WorldCreationSettings`` to only apply your generator changes to nether worlds.
...
State. To register it, simply call :javadoc:`org.spongepowered.api.GameRegistry#register(java.lang.Class, T)` with
