...
Of course, this isn't the only way to load a file. The builder also has the method ``setURL(URL)``, in case you might'
want to load a resource without using a ``File`` object. The following example represents loading a config from the
plugin jar file.
