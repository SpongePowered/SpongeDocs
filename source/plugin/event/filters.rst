...
Event type filters are method annotations that are applied to your listener method along with the
:javadoc:`org.spongepowered.api.event.Listener` annotation and provide several filters based on the type or state of
the event.
...
**@Include/@Exclude**
These two parameters are used in conjunction with listening for supertype events such as
:javadoc:`org.spongepowered.api.event.entity.AffectEntityEvent` while narrowing the events that you receive to just a
subset of the events extending the event you're listening for.
...
This listener would normally be called for all events extending InteractBlockEvent. However, the
:javadoc:`org.spongepowered.api.event.filter.type.Exclude` annotationte
will prevent your listener from being called for the
:javadoc:`org.spongepowered.api.event.block.InteractBlockEvent.Primary` event (leaving just the
:javadoc:`org.spongepowered.api.event.block.InteractBlockEvent.Secondary` event).

An example with :javadoc:`org.spongepowered.api.event.filter.type.Include` could be:
...
only recieve :javadoc:`org.spongepowered.api.event.entity.DamageEntityEvent` and
:javadoc:`org.spongepowered.api.event.entity.DestructEntityEvent`\ s.
...
However you can change this behavior to one of three states depending on the :javadoc:`org.spongepowered.api.util.
Tristate` value in the :javadoc:`org.spongepowered.api.event.filter.IsCancelled` annotation.
...
the type of your parameter (This is equivalent to :javadoc:`org.spongepowered.api.event.cause.Cause#first(java.lang.
Class)`). If no object is found matching this parameter then your listener is not called.
...
**@Last** This is similar to ``@First`` however it instead makes a call to
:javadoc:`org.spongepowered.api.event.cause.Cause#last(java.lang.Class)`.
...
the one of the type specified by the annotation parameter (This is equivalent to
:javadoc:`org.spongepowered.api.event.cause.Cause#before(java.lang.Class)`).
...
**@After** This is similar to ``@Before``, but it instead uses
:javadoc:`org.spongepowered.api.event.cause.Cause#after(java.lang.Class)`.

**@All** This parameter source annotation requires that the annotated parameter be an array
type. The returned array will be equivalent to the contents of calling
:javadoc:`org.spongepowered.api.event.cause.Cause#allOf(java.lang.Class)`. By default if the returned array would be
...
:javadoc:`org.spongepowered.api.event.cause.Cause#root()`. It also performs an additional check that the type of the
root object matches the type of your parameter.
...
parameter (This is equivalent to :javadoc:`org.spongepowered.api.event.cause.Cause#get(java.lang.String, java.lang.Class)`).
Additionally, the found object must match the type of the parameter. If no object is found meeting these criteria, then
...
**@Supports**
This parameter filter may be applied to any parameter type which is a :javadoc:`org.spongepowered.api.data.DataHolder`.
It takes a class extending :javadoc:`org.spongepowered.api.data.manipulator.DataManipulator` as its parameter and
validates that the annotated ``DataHolder`` supports the given ``DataManipulator``. This validation is equivalent to
:javadoc:`org.spongepowered.api.data.value.mutable.CompositeValueStore#supports(java.lang.Class<? extends H>)`.
