...
your plugin by finding its main class, annotated by the :javadoc:`org.spongepowered.api.plugin.Plugin` annotation that
holds its general information, and sends a new instance of it to the manager. The manager then keeps that instance in
its own collection that you can look into and pull from using methods provided by itself, thus allowing you to easily
interact with another loaded plugin if you so desire.

The PluginManager Class
~~~~~~~~~~~~~~~~~~~~~~~

Public methods inside the :javadoc:`org.spongepowered.api.plugin.PluginManager` are used to grab information about the
current collection of loaded plugins, alongside their instances. The plugins are stored inside a
:javadoc:`org.spongepowered.api.plugin.PluginContainer` (discussed in next section) to allow for an easy center of
information about the specific plugin. As an example, you can use the ``PluginManager`` to communicate with another
plugin, grabbing its instance and using the methods it offers to provide compability or extended features by means of
your calling plugin.
...
The service manager also holds an instance of the server's ``PluginManager``. Simply use the method :javadoc:`org.
spongepowered.api.service.ServiceManager#provide(java.lang.Class)`, passing the ``PluginManager``\ 's class
(``PluginManager.class``) as a parameter.
...
The :javadoc:`org.spongepowered.api.Sponge` class works similarly to ``Game``, with the exception that since ``Sponge``
contains static methods. It can be accessed anywhere throughout your plugin. You also do not need to store an instance
of it, as you would need to do with ``Game``.
...
become wrong as a result of this change, provided you've used the method
:javadoc:`org.spongepowered.api.plugin.PluginContainer#getName()` to get its name.
...
.. note::
    ...
    :javadoc:`org.spongepowered.api.plugin.PluginContainer#getInstance()` will return as an ``Object``. You need to
    cast it as the target plugin after obtaining it from the container.
